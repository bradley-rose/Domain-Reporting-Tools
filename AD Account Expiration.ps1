...
$Date = ((Get-Date).ToUniversalTime()).Ticks - ([DateTime]"January 1, 1601").Ticks #Converts the date to the value that LDAP uses to calculate its queries.

Get-ADUser -LDAPFilter "(&(objectCategory=person)(objectClass=user)(accountExpires>=1)(accountExpires<=$Date))" -Properties DisplayName,Description,HomeDirectory,AccountExpirationDate | Select-Object DisplayName,Description,HomeDirectory,AccountExpirationDate | Export-Csv "C:\Users\$loggeduser\Desktop\expiredUsers.csv"
